...
- [ ] **Frontend: Create Compliance Mapping Page** (AC: 8, 10, 11, 12)
  ...
  - [ ] Fetch all controls via `GET /api/v1/controls` (Epic 1 endpoint)
  - [ ] Display control list in left sidebar or dropdown selector
  - [ ] Implement control selection handler
  - [ ] Fetch existing mappings via `GET /api/v1/mappings/control/{id}` using React Query
  - [ ] Fetch all regulatory requirements via `GET /api/v1/regulatory-frameworks`
  - [ ] Calculate available requirements: all requirements - mapped requirements
  - [ ] Render `DualListSelector` with available and mapped requirements
  - [ ] Implement "Add Mapping" handler: calls `POST /api/v1/mappings`, optimistic update, success/error toasts
  - [ ] Implement "Remove Mapping" handler: calls `DELETE /api/v1/mappings`, optimistic update, toasts
  - [ ] Implement view toggle (Control vs. Requirement perspective) with state management
  - [ ] Implement requirement perspective: fetch all requirements, select requirement, fetch mappings, show controls in dual-list
  - [ ] Add loading states and skeleton UI
  - [ ] Ensure Admin-only access (redirect non-Admin to 403 page)

- [ ] **Frontend: Implement React Query Hooks** (AC: 10, 11)
  - [ ] Create `useMappings` hook in `frontend/hooks/useMappings.ts`
  - [ ] Implement `useControlMappings(controlId)` with React Query
  - [ ] Implement `useRequirementMappings(requirementId)` with React Query
  - [ ] Implement `useCreateMapping()` mutation with optimistic updates and cache invalidation
  - [ ] Implement `useDeleteMapping()` mutation with optimistic updates and cache invalidation
  - [ ] Set cache TTL to 60 seconds

- [ ] **Frontend: Add Toast Notifications** (AC: 10, 11)
  - [ ] Import Sonner toast library (already in package.json)
  - [ ] Add success toasts: "✅ Mapping created", "✅ Mapping removed"
  - [ ] Add error toasts with error messages from API responses
  - [ ] Ensure toasts appear on all mapping operations

- [ ] **Frontend: Write Component Tests** (AC: 9, 10, 11)
  - [ ] Create `frontend/__tests__/components/custom/DualListSelector.test.tsx`
  - [ ] Test rendering with available and selected items
  - [ ] Test add action: select items, click "Add", verify onAdd callback
  - [ ] Test remove action: select items, click "Remove", verify onRemove callback
  - [ ] Create `frontend/__tests__/app/(dashboard)/admin/compliance-mapping/page.test.tsx`
  - [ ] Test page renders for Admin user
  - [ ] Test control selection triggers mapping fetch
  - [ ] Test add mapping flow with mocked API
  - [ ] Test remove mapping flow with mocked API
  - [ ] Test view toggle between Control and Requirement perspectives
  - [ ] Run tests and verify all pass
